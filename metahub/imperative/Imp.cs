    ...
    public class Imp
    {
        ...
        public void process(Node expression, Scope scope)
        {
            switch (expression.type)
            {
                ...
                case Node_Type.block:
                    block_expression(((metahub.meta.types.Block)expression).children, scope);
                    break;
                ...
                case metahub.meta.types.Node_Type.path:
                    block_expression(((metahub.meta.types.Reference_Path)expression).children, scope);
                    break;
        ...
        public Expression translate(Node expression, Scope scope = null)
        {
            switch (expression.type)
            {
                ...
                case Node_Type.path:
                    return convert_path((metahub.meta.types.Reference_Path)expression, scope);

                case Node_Type.array:
                    return new Create_Array(translate_many(((metahub.meta.types.Array_Expression)expression).children, scope));

                case Node_Type.block:
                    return new Create_Array(translate_many(((metahub.meta.types.Block)expression).children, scope));
        ...
        public Expression convert_path(metahub.meta.types.Reference_Path expression, Scope scope = null)
        {
            ...
            {
                rail = ((metahub.meta.types.Property_Reference)path[0]).tie.get_abstract_rail();
            }
            //else
            //{
            //    rail = ((metahub.meta.types.Array_Expression)path[0])
            //}
            foreach (var token in path)
            {
                switch (token.type)
                {
                    ...
                    case Node_Type.variable:
                        var variable = (metahub.meta.types.Variable) token;
